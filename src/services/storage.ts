...
function migrateTuneMap(raw: Record<string, Tune>): Record<string, Tune> {
  ...
  Object.values(canonical).forEach((tune) => { const key = canonicalKey(tune); const existing = byIdentity[key]; if (!existing || new Date(tune.updatedAt || tune.createdAt || 0).getTime() >= new Date(existing.updatedAt || existing.createdAt || 0).getTime()) byIdentity[key] = tune; });
  return byIdentity;
}
...
export const StorageService = {
  ...
  deleteVehicle(id: string): void { localStorage.setItem(STORAGE_KEYS.VEHICLES, JSON.stringify(this.getVehicles().filter(v => v.id !== id))); const tunes = Object.values(this.getTunes()).filter(t => t.vehicleId !== id).reduce((m, t) => { m[canonicalKey(t)] = t; return m; }, {} as Record<string,Tune>); localStorage.setItem(STORAGE_KEYS.TUNES, JSON.stringify(tunes)); },

...
export const CloudSync = {
  ...
  async sync(): Promise<{ status: SyncStatus; user: User | null }> {
    ...
    const vehiclesToUpload = localVehicles.filter(v => {
      const cloud = cloudVehicleMap.get(v.id);
      return !cloud || stamp(v) >= Date.parse(cloud.updated_at || '') || !cloud.data;
    });
    ...
    const tunesToUpload = localTunes.filter(t => {
      const cloud = cloudTuneMap.get(`${t.vehicleId}__${t.discipline}`);
      return !cloud || stamp(t) >= Date.parse(cloud.updated_at || '');
    });
